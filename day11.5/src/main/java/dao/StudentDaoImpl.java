...
public class StudentDaoImpl implements IStudentDao {
	...
	public String assignStudentAddress(int studentId, Address address) {
		...
		try {
			...
			if (student != null) {
				//address : TRANSISENT , student : PERSISTENT 
				address.setOwner(student);//address--->student
				session.persist(address);//since no cascading : one has to EXPLICTLY save address
				mesg = "address assignment done";
			}
			tx.commit();//DML : insertion of a record : address tbl(PK--->FK of student tbl)
		} catch (RuntimeException e) {
			if (tx != null)
				tx.rollback();
			throw e;
